...
local chests_boundary = 0

local chests = {}
local filling = false
local filepath = minetest.get_worldpath()..'/airdrop'
...
local fill_chest = function(pos)
	...
	minetest.env:add_node(pos,{name='army:airdrop', inv=invcontent})	
...
function random_chests.refill(i)
	...
	while i <= table.getn(chests) do
		if chests[i] then
			...
			if (not n:match("army:airdrop")) and n ~= "ignore" then
	...
	filling = false
	random_chests.save()
	print("finished filling airdrops")
end
...
function random_chests.setrefill(mode, interval)
	...
	elseif mode == "nodetimer" then
		...
		local chest = {}
		for k,v in pairs(minetest.registered_nodes["army:airdrop"]) do
			chest[k] = v
		end
		...
		minetest.register_node(":army:airdrop", chest)
